...
- [ ] 3.2.1 实现同步引擎 (需求 5)
  ...
  - 支持同步粒度控制（all、requirements、tasks、single）(需求 16)
...
### 6.2 init 命令
- [ ] 6.2.1 实现 init 命令 (需求 6)
  - 在 `src/cli/commands/init.ts` 中创建 initCommand
  - 创建默认的 .specbridge.yaml 配置文件
  - 支持 --force 选项覆盖现有配置
  - 在已有配置时提示用户
  - 提示用户添加 .specbridge/ 到 .gitignore (需求 20)
  - 显示配置示例和下一步操作

### 6.3 sync 命令
- [ ] 6.3.1 实现 sync 命令 (需求 6, 16)
...
### 7.1 TypeScript 配置
- [ ] 7.1.1 验证和优化 TypeScript 配置 (需求 9)
...
### 7.2 代码质量工具
- [ ] 7.2.1 配置 ESLint 和 Prettier (需求 12)
